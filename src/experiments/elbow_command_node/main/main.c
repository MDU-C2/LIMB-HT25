...
void command_task(void* pvParameter) {
  ...
  while (1) {
    ...
    *(float*)can_data = target_angle;
    ...
    if (ret == ESP_OK) {
      ESP_LOGI(TAG, ">>> Sent command: up down target angle = %f degrees",
               target_angle);
    } else {
      ESP_LOGE(TAG, "Failed to send up down CAN message: %s",
               esp_err_to_name(ret));
    }
    ret = can_send(CAN_ID_ROBOT_UPPER_ARM_ROTATION_ACTUATION, can_data, 8);
    if (ret == ESP_OK) {
      ESP_LOGI(TAG, ">>> Sent command: rotation target angle = %f degrees",
               target_angle);
    } else {
      ESP_LOGE(TAG, "Failed to send rotation CAN message: %s",
               esp_err_to_name(ret));
    }
  ...
  while (1) {
    if (can_receive(&rx_id, msg_rx, &rx_len, 1000) == ESP_OK) {
      if (rx_id == CAN_ID_ROBOT_ELBOW_UP_DOWN_POTENTIOMETER) {
        ...
        ESP_LOGI(TAG, "<<< Received elbow status: current angle = %.1f°",
                 angle);
      } else if (rx_id == CAN_ID_ROBOT_UPPER_ARM_ROTATION_POTENTIOMETER) {
        float angle = *(float*)msg_rx;
        ESP_LOGI(
            TAG,
            "<<< Received upper arm rotation status: current angle = %.1f°",
            angle);
      } else {
