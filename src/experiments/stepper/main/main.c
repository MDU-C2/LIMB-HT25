...
#define POT_ADC_CHANNEL \
  ADC_CHANNEL_2  // Potentiometer ADC channel (or ADC_CHANNEL_MAX if not used)
...
AdcMgrReadResults s_adc_read_results = {
    .channel_buffers[POT_ADC_CHANNEL] =
        (AdcMgrChannelBuffer){
            .data = s_potentiometer_adc_underlying_buf,
            .capacity = 1024,
        },
};
AdcMgrChannelBuffer* s_read_potentiometer_values_buf =
    &s_adc_read_results.channel_buffers[POT_ADC_CHANNEL];
...
static void stepper_control_task(void* pvParameters) {
  ...
  ESP_LOGI(TAG, "Control task started (update period: %d ms)",
           UPDATE_PERIOD_MS);

  while (1) {
    xSemaphoreTake(s_latest_potentiometer_mutex, portMAX_DELAY);
    // Update stepper control loop
    stepper_update(*stepper_handle, dt, s_read_potentiometer_values_buf->data,
                   s_read_potentiometer_values_buf->length);
    xSemaphoreGive(s_latest_potentiometer_mutex);
...
static void status_task(void* pvParameters) {
  ...
  while (1) {
    ...
    PotentiometerAngle current_pot_angle =
        stepper_get_current_angle_deg(*stepper_handle);
    PotentiometerAngle target_pot_angle =
        stepper_get_target_angle_deg(*stepper_handle);
    JointAngle current_joint_angle =
        to_joint_angle(&foo->potentiometer, current_pot_angle);
    JointAngle target_joint_angle =
        to_joint_angle(&foo->potentiometer, target_pot_angle);
    ...
    ESP_LOGI(
        TAG,
        "Status: current(pot)=%.2f°, target(pot)=%.2f°, current(joint)=%.2f°, "
        "target(joint)=%.2f°, vel=%.2f°/s, moving=%d, feedback=%d",
        current_pot_angle.degree, target_pot_angle.degree,
        current_joint_angle.degree, target_joint_angle.degree, velocity,
        is_moving, has_feedback);
...
static void test_sequence_task(void* pvParameters) {
  ...
  bool has_feedback = stepper_has_position_feedback(*stepper_handle);
  ESP_LOGI(TAG, "Test 1: Position feedback available: %s",
           has_feedback ? "YES" : "NO");
  if (has_feedback) {
  ...
  stepper_set_target_angle_deg(
      *stepper_handle,
      to_potentiometer_angle(&cfg->potentiometer, (JointAngle){45.F}));
  ...
  stepper_set_target_angle_deg(
      *stepper_handle,
      to_potentiometer_angle(&cfg->potentiometer, (JointAngle){0}));
  vTaskDelay(pdMS_TO_TICKS(5000));  // Wait for movement
  ...
  stepper_set_target_angle_deg(
      *stepper_handle,
      to_potentiometer_angle(&cfg->potentiometer, (JointAngle){30.F}));
  ...
  for (int i = 0; i < 20; i++) {
    ...
    stepper_set_target_angle_deg(
        *stepper_handle, to_potentiometer_angle(&cfg->potentiometer, angle));
  ...
  stepper_set_target_angle_deg(*stepper_handle,
                               (PotentiometerAngle){285.F / 2.F});
  vTaskDelay(pdMS_TO_TICKS(5000));

  ESP_LOGI(TAG, "=== Test Sequence Complete ===");
  ESP_LOGI(
      TAG,
      "Stepper will continue running. Check status logs for current state.");
...
void app_main(void) {
  ...
  AdcMgrChannelConfig adc_channel_configs[] = {(AdcMgrChannelConfig){
  ...
  xSemaphoreGive(s_latest_potentiometer_mutex);
  ESP_LOGI(TAG, "Read %d initial adc values",
           s_adc_read_results.channel_buffers[POT_ADC_CHANNEL].length);

  {
    ...
    Potentiometer potentiometer = {
        ...
        // The range has to align with the
        // min/max_join_angle_as_potentiometer_angles.
        ...
        .min_joint_angle_as_potentiometer_angle =
            (PotentiometerAngle){285.F / 2.F - 90.F},
    ...
    // Initialize stepper
    esp_err_t ret = stepper_init(&config, s_read_potentiometer_values_buf->data,
                                 s_read_potentiometer_values_buf->length,
                                 &s_stepper_handle);
    if (ret != ESP_OK) {
  ...
  xTaskCreate(stepper_control_task, "stepper_ctrl", 4096, &s_stepper_handle, 5,
              NULL);
