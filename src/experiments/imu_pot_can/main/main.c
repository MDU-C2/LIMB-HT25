...
static void send_pot_data(int raw_value) {
  ...
  esp_err_t ret = twai_transmit(&message, pdMS_TO_TICKS(100));
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "Failed to send potentiometer data: %s",
             esp_err_to_name(ret));
  }
}
...
void app_main(void) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to initialize potentiometer: %s",
             esp_err_to_name(ret));
    imu_deinit();
    return;
  }
  ...
  ESP_LOGI(TAG, "Application started. Sending data every %d ms",
           SEND_PERIOD_MS);
}
