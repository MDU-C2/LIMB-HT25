...
void SendImuDataTask(void* arg) {
  ...
  while (1) {
    ...
    if (ret == ESP_OK) {
      ...
      //   gyro.x, gyro.y, gyro.z, accel.x, accel.y, accel.z (each int16_t,
      //   little-endian)
      ...
      memcpy(sensor_data + 0, &data.gyro.x,
             2);  // gyro.x (int16_t, little-endian)
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "Initializing IMU: SDA=%d, SCL=%d, addr=0x%02X", config.sda_pin,
           config.scl_pin, config.sensor_addr);

  esp_err_t ret = imu_init(&config);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "IMU init failed: %s (0x%x)", esp_err_to_name(ret), ret);
    ESP_LOGE(TAG, "Check:");
    ESP_LOGE(TAG, "  1. I2C pins are correct (SDA=%d, SCL=%d)", config.sda_pin,
             config.scl_pin);
