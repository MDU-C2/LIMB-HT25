...
def open_serial(port, baudrate):
    ...
    # Avoid some boards/macOS behaving oddly with control lines
    # Prevent auto-reset / bootloader dance (important on ESP boards)
    ser.dtr = False
    ser.rts = False
...
def read_and_print_imu(port=None, baudrate=115200, method='estimate', visualize=True):
    ...
    try:
        while True:
            # Read a complete line using readline()
            try:
        ...
        print(f"Serial glitch/disconnect: {e}")
    except Exception as e:
        print(f"\nError: {e}")
        import traceback
        traceback.print_exc()
