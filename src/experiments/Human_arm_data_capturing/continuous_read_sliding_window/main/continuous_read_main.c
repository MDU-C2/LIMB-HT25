...
#define WINDOW_SIZE \
  800  // The total number of samples in each data window sent for processing.
#define STEP_SIZE \
  400  // The number of NEW samples to acquire before creating the next window.
// This results in a 50% overlap (400 samples) between consecutive windows.

// --- ADC & DMA Parameters ---
#define ADC_SAMPLING_FREQ \
  4000  // The speed at which the ADC will capture samples, in Hertz (Hz).
#define DMA_TRANSFER_LEN \
  256  // The size of each data block transferred from the ADC to memory, in
       // bytes. Since each 12-bit sample requires 2 bytes, this block holds 128
       // samples. A smaller size reduces latency but increases CPU overhead
       // slightly.
...
#define EXAMPLE_ADC_ATTEN \
  ADC_ATTEN_DB_12  // Use the widest voltage measurement range (~0-3.0V).
#define EXAMPLE_ADC_BIT_WIDTH \
  SOC_ADC_DIGI_MAX_BITWIDTH  // Use the highest hardware-supported resolution
                             // (12-bit).
...
static adc_channel_t channel[1] = {
    ADC_CHANNEL_2};                 // The ADC channel (GPIO pin) to read from.
static TaskHandle_t s_task_handle;  // A handle to the main task, used for
                                    // notifications from the ISR.
...
// top for better readability. The actual function bodies are defined at the
// end.
...
// NOTE: ISRs must be extremely fast. Do not perform complex calculations or
// blocking operations here.
static bool IRAM_ATTR s_conv_done_cb(adc_continuous_handle_t handle,
                                     const adc_continuous_evt_data_t* edata,
                                     void* user_data) {
  BaseType_t mustYield = pdFALSE;
  // Notify the main task that new data is available. This is a very fast,
...
void app_main(void) {
  ...
  bool do_calibration = adc_calibration_init(EXAMPLE_ADC_UNIT, channel[0],
  ...
  // Start the ADC. From this point on, the hardware will be capturing data in
  // the background.
  ESP_ERROR_CHECK(adc_continuous_start(handle));

  // --- Main Processing Loop ---
  while (1) {
    // Wait here indefinitely until notified by the ISR. This consumes zero CPU
    // while waiting.
    ...
    while (1) {
      ...
      ret = adc_continuous_read(handle, dma_result_buffer, DMA_TRANSFER_LEN,
                                &ret_num, 0);
      ...
      if (ret == ESP_OK) {
        ...
        for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES) {
          ...
          adc_digi_output_data_t* p =
              (adc_digi_output_data_t*)&dma_result_buffer[i];
          ...
          // 3. Check if we have gathered enough new samples to create a new
          // window.
          if (s_new_samples_count >= STEP_SIZE) {
            s_new_samples_count = 0;  // Reset the counter for the next step.

            // --- Window Reconstruction ---
            // This logic creates a flat, linear copy of the data from the
            // circular buffer. It correctly handles the "wrap-around" case
            // where the window is split in two parts.
            int start_index =
                (s_write_index - WINDOW_SIZE + WINDOW_SIZE) % WINDOW_SIZE;

            if (start_index + WINDOW_SIZE <= WINDOW_SIZE) {
              // Simple case: The window is in one contiguous block.
              memcpy(s_window_to_send, &s_emg_circular_buffer[start_index],
                     WINDOW_SIZE * sizeof(float));
            } else {
              // Complex case: The window wraps around. It must be copied in two
              // chunks.
              int first_chunk_size = WINDOW_SIZE - start_index;
              memcpy(s_window_to_send, &s_emg_circular_buffer[start_index],
                     first_chunk_size * sizeof(float));
              memcpy(&s_window_to_send[first_chunk_size], s_emg_circular_buffer,
                     (WINDOW_SIZE - first_chunk_size) * sizeof(float));
            }

            // --- Debugging Output ---
            ESP_LOGI(TAG,
                     "Window created. Start index: %d. Size: %d. Step: %d.",
                     start_index, WINDOW_SIZE, STEP_SIZE);

            // =========================================================================
            // == 4. PLACEHOLDER FOR BLUETOOTH DATA TRANSMISSION
            // =========================================================================
            // At this point, `s_window_to_send` contains the complete
            // 800-sample voltage window, perfectly ordered and ready for
            // transmission.
            //
        ...
        // If adc_continuous_read returns a timeout, it means the DMA buffer is
        // empty. We can break from this inner loop and go back to sleep,
  ...
  // In this specific application, the main loop runs forever. However, it's
  // good practice to include cleanup code for more complex applications where
  // tasks might be stopped and started.
...
// This function configures and initializes the ADC for continuous (DMA-based)
// reading.
static void continuous_adc_init(adc_channel_t* channel, uint8_t channel_num,
                                adc_continuous_handle_t* out_handle) {
  adc_continuous_handle_t handle = NULL;
  ...
  adc_continuous_handle_cfg_t adc_config = {
      .max_store_buf_size =
          4096,  // Increased to handle high-frequency data without overflow.
...
// This function sets up the ADC calibration based on values stored in the
// chip's eFuse.
static bool adc_calibration_init(adc_unit_t unit, adc_channel_t channel,
                                 adc_atten_t atten,
                                 adc_cali_handle_t* out_handle) {
