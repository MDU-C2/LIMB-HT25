...
void app_main(void) {
  ...
  do_calibration = adc_calibration_init(EXAMPLE_ADC_UNIT, channel[0],
      ...
      if (ret == ESP_OK) {
        ...
        for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES) {
          ...
          if (do_calibration) {
            ...
            ESP_LOGI(TAG,
                     "Channel: %" PRIu32 ", Raw: %" PRIu32 ", Voltage: %.2f V",
                     chan_num, data, voltage_V);
          } else {
            // if there is no calibration, store raw value
            ESP_LOGI(TAG, "Channel: %" PRIu32 ", Raw: %" PRIu32, chan_num,
                     data);
          }
        }
        /**
