...
static esp_err_t imu_register_read(uint8_t reg_addr, uint8_t* data,
  ...
  return i2c_master_write_read_device(
      I2C_MASTER_NUM, LSM6DSO32_ADDR, &reg_addr, 1, data, len,
      I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}

static esp_err_t imu_register_write(uint8_t reg_addr, uint8_t data) {
  uint8_t write_buf[2] = {reg_addr, data};
  return i2c_master_write_to_device(I2C_MASTER_NUM, LSM6DSO32_ADDR, write_buf,
                                    sizeof(write_buf),
                                    I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
...
static esp_err_t imu_init(void) {
  ...
  ret = imu_register_write(LSM6DSO32_CTRL1_XL,
                           0x50);  // 26 Hz ODR, high-performance mode
...
static void imu_tx_task(void* arg) {
  ...
  while (1) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Failed to read IMU data: %s (0x%x)", esp_err_to_name(ret),
               ret);
    ...
    ret =
        can_send(CAN_ID_IMU_DATA, (uint8_t*)&can_payload, sizeof(can_payload));
    if (ret != ESP_OK) {
