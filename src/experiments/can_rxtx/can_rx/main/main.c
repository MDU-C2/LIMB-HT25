 ...
 */
static void parse_imu_frame(const uint8_t *data, uint8_t len, imu_can_payload_t *payload)
{
    if (len >= 8) {
    ...
    while(1) {
        ...
        if (ret == ESP_OK) {
            ...
            if (can_id == CAN_ID_IMU_DATA) {
                ...
                ESP_LOGI(TAG, "IMU Data: accel_x=%.3f, accel_y=%.3f, gyro_x=%.3f, gyro_y=%.3f",
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "CAN initialized: TX=GPIO%d, RX=GPIO%d, %d bps",
        CAN_TX_PIN, CAN_RX_PIN, CAN_BAUDRATE);
