  ...
  while (1) {
    ...
    if (can_receive(&rx_id, msg_rx, &rx_len, 100) == ESP_OK) {
      ...
      if (rx_id == ID_SENSOR_DATA && rx_len == NUM_FINGERS) {
        ...
        ESP_LOGI(TAG, "Angles %d - %d - %d - %d - %d ", (int)msg_rx[0],
                 (int)msg_rx[1], (int)msg_rx[2], (int)msg_rx[3],
                 (int)msg_rx[4]);

      } else {
        ESP_LOGI(TAG,
                 "CAN RX: Mensaje con ID 0x%X y Longitud %d ignorado "
                 "(Esperado: ID 0x020, Len 5).",
                 rx_id, rx_len);
      }
    }
