 ...
 *---------------------------------------------------------------*/
// Defines the number of FSR sensors used (corrected to 4 based on array initialization).
...
//Diferentes setpoint por dedo y que sean adaptativas, diferentes formas de vaso, diferentes materiales
...
while (1) {
        ...
        for (int i = 0; i < NUM_FINGERS; i++) {
            ...
            if (i == 4) { 
                ...
                if (filtered_voltage_mV[i] == 0.0) {
                    ...
                    filtered_voltage_mV[i] = ALPHA_LPF * (float)voltage_mV[i] + (1.0 - ALPHA_LPF) * filtered_voltage_mV[i];
                }
                ...
                // ESP_LOGI(TAG, "D1 Control | V_mV:%d | FILT_mV:%.0f | ERR_mV:%.0f | Ts:%.4fs | Delta_A:%.1f° | New_A:%d°", 
                //     voltage_mV[i], 
        ...
        ESP_LOGI(TAG, "D1: %d, D2: %d, D3: %d, D4: %d, D5: %d",  
                voltage_mV[0], voltage_mV[1], voltage_mV[2], voltage_mV[3], voltage_mV[4]);
        // esp_err_t err1 = can_send(ID_SENSOR_DATA, tx_data, 5);
        // if (err1 == ESP_OK) {
        //     ESP_LOGI(TAG, "CAN TX OK [0x%03X] (DLC=5): A1:%d A2:%d A3:%d A4:%d A5:%d", 
        //         ID_SENSOR_DATA, 
