...
static const char* const TAG = "Continuous Servo";

typedef struct {
  portMUX_TYPE spinlock;
  ContinuousServoConfig cfg;
...
static uint32_t us_to_duty(const ContinuousServoConfig* servo, uint16_t us) {
  us = LIMB_CLAMP(
...
static ServoContext* servo_get_context(ContinuousServoHandle handle) {
...
void stop_motor(ContinuousServoHandle handle) {
  ServoContext* ctx = servo_get_context(handle);
  continuous_servo_apply_velocity(handle, (AngularVelocity){0});
...
void continuous_servo_apply_velocity(ContinuousServoHandle handle,
  ...
  if (velocity.dps == 0.F || ctx->estop_active) {
    continuous_servo_apply_pulse_width_as_velocity(handle,
                                                   ctx->cfg.motionless_pw);
    return;
  }
  ...
  if (ctx->cfg.direction == CONTINUOUS_SERVO_DIR_REVERSE) {
  ...
  continuous_servo_apply_pulse_width_as_velocity(
