...
static_assert(sizeof(s_temp_imu) == kImuBufSize,
              "We expect that the IMU micro packet and the BLE IMU "
              "characteristic have the same size.");

/**
 * @brief Main BLE synchronization and dispatch task.
 * This task waits for event bits from ADC and IMU services.
 * It ensures that data is sent as soon as it is ready, maintaining high
 * throughput.
 */
static void ble_sync_send_task(void* pvParameters) {
  ...
  while (1) {
    ...
    EventBits_t bits = xEventGroupWaitBits(sync_group, STREAM_BITS,
    ...
    if (bits & ADC_EMG_STREAM_BIT) {
      ...
      CharacteristicBuffer master =
          get_emg_buf();  // Get pointer to GATT characteristic buffer
    ...
    if (now - last_log_time >= 5000000) {
      ...
      ESP_LOGI("DIAG", "Last Data -> EMG1:%u | PIEZO:%u", s_temp_emg.data[39],
               s_temp_piezo.data[9]);

      ESP_LOGI("DIAG", "IMU -> Acc [x, y, z]: [%.2f, %.2f, %.2f] mg",
               s_temp_imu.imu_data[0], s_temp_imu.imu_data[1],
               s_temp_imu.imu_data[2]);
      ESP_LOGI("DIAG",
               "IMU -> Gyro [pitch, roll, yaw]: [%.2f, %.2f, %.2f] mdps",
               s_temp_imu.imu_data[3], s_temp_imu.imu_data[4],
               s_temp_imu.imu_data[5]);
      ESP_LOGI("DIAG", "*******************");
 ...
 */
esp_err_t ble_service_start(EventGroupHandle_t event_group,
  ...
  BaseType_t ret = xTaskCreatePinnedToCore(
      ble_sync_send_task, "ble_sync_task", 4096, &params,
