...
from sensor_packet_serialization import deserialize_packet_data
...
class TestDeserializePacketData(unittest.TestCase):
    ...
    # [1, 0, 2, 0, ..., 24, 0]
    imu_le_data = bytes(
        functools.reduce(lambda acc, i: [*acc, i, 0], range(2, 25), [1, 0]),
    )
    # [0, 1, 0, 2, 0, ..., 0, 24]
    imu_be_data = bytes(
    ...
    def test_big_endian_imu(self) -> None:
        """Test deserialize_packet_data with mock IMU data in big endian format."""
        result = deserialize_packet_data(
        ...
        # (meaning we start at 256 and count in steps of 256).
        target = [
            [
