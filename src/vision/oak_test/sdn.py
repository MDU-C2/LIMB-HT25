...
# Use model from DepthAI model zoo
# Available models: yolov6-nano, yolov8n, yolov9-tiny, etc.
#modelDescription = dai.NNModelDescription("luxonis/yolov10-nano:coco-512x288")
...
class SpatialVisualizer(dai.node.HostNode):
    ...
    def process(self, depthPreview, detections, rgbPreview):
        ...
        # Filter detections to only include cups
        cup_detections = [d for d in detections.detections if d.labelName == "cup"]
        self.displayResults(rgbPreview, depthFrameColor, cup_detections)
