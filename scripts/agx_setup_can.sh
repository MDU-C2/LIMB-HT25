#! /bin/sh
# Setting up CAN for the Jetson AGX Orin following NVIDIA's developer guide:
...
usage() {
  echo "This script sets up a virtual CAN interface on the Jetson AGX Orin."
  echo ""
...
controller1_status=$(cat /proc/device-tree/mttcan\@c310000/status)
if [ "$controller1_status" != "okay" ]; then
  echo "FATAL: CAN controller1 is not reporting okay!" >&2
  exit 1
fi
controller2_status=$(cat /proc/device-tree/mttcan\@c320000/status)
if [ "$controller2_status" != "okay" ]; then
  echo "FATAL: CAN controller 2 is not reporting okay!" >&2
  exit 1
fi
